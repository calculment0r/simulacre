/* ── Proxy god mode (Cloudflare Worker) ───────────────────────────
   Deux rôles, deux secrets côté serveur (jamais dans le repo) :
   1) Proxy Anthropic  → POST /            (secret ANTHROPIC_KEY)
   2) Partage variantes → GET/POST /variants (secret GITHUB_TOKEN)

   Personne (ni Nico ni Eric ni les autres) n'entre de clé ni de token.
   Les variantes vivent dans le repo GitHub (site/data/variants.json),
   donc tous les contributeurs les voient. PC éteint OK.

   SECRETS À AJOUTER DANS CLOUDFLARE (Worker → Settings → Variables and Secrets) :
   - ANTHROPIC_KEY = sk-ant-...            (déjà fait)
   - GITHUB_TOKEN  = github_pat_...        (fine-grained, Contents: read+write
                                            sur calculment0r/simulacre)
   ─────────────────────────────────────────────────────────────── */

const ALLOW_ORIGIN = 'https://calculment0r.github.io';
const REPO = 'calculment0r/simulacre';
const VPATH = 'site/data/variants.json';
...
export default {
  async fetch(request, env) {
    ...
    // ── partage des variantes via GitHub (token caché) ──────────────
    if (url.pathname.endsWith('/variants')) {
      ...
      const api = `https://api.github.com/repos/${REPO}/contents/${VPATH}`;
      ...
      if (request.method === 'POST') {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const put = await fetch(api, {
            method: 'PUT',
            headers: gh,
            body: JSON.stringify({ message: 'variantes god mode', content: b64(body), branch: 'main', sha }),
          });
